...
@Component({
  selector: 'app-financial-processing-voucher-treaty',
  templateUrl: './treaty.component.html',
})
export class FinancialProcessingVoucherTreatyComponent implements OnInit {
  listOfData:any[] = [];
  ...
  getDataList(id:number = null) { // 获取单位下的数据
    this.loading = true;
    let url:string = id ?  `/api/treaty_pay/finished/${id}` : '/api/treaty_pay/finished/all';
