...
@Component({
  selector: 'app-my-apply-for-pass',
  templateUrl: './my-apply-for-pass.component.html',
  styles: []
})
export class MyApplyForPassComponent implements OnInit {
  ...
  getDataList() { // 获取单位下的数据
    this.loading = true;
    this.settingConfigService.get('/api/contract/pay/pass/my', this.pageOption).subscribe((res:ApiData) => {
      console.log(res);
      this.loading = false;
      if(res.code === 200) {
