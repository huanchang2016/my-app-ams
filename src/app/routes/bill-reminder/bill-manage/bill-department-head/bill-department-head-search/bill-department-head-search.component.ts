...
@Component({
  selector: 'app-bill-department-head-search',
  templateUrl: './bill-department-head-search.component.html',
  styles: [
  ]
})
export class BillDepartmentHeadSearchComponent implements OnInit {
  ...
  billList(option) {
    ...
    this.settingsConfigService.post('/api/bill/department_head ', option).subscribe((res: ApiData) => {
      console.log('billList res', res.data);
      if (res.code === 200) {
        console.log('发票列表(部门负责人)');
