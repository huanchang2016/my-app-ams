...
@Component({
  selector: 'app-contract-department-head-search',
  templateUrl: './contract-department-head-search.component.html',
  styles: [
  ]
})
export class ContractDepartmentHeadSearchComponent implements OnInit {
  ...
  listRequest(option) {
    ...
    this.settingsConfigService.post('/api/contract_pay/department_head', option).subscribe((res: ApiData) => {
      console.log('listRequest res', res.data);
      if (res.code === 200) {
        console.log('支付列表（部门负责人）');
