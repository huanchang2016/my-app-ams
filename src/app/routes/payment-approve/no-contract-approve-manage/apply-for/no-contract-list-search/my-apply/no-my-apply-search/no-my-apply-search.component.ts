...
@Component({
  selector: 'app-no-my-apply-search',
  templateUrl: './no-my-apply-search.component.html',
  styles: [
  ]
})
export class NoMyApplySearchComponent implements OnInit {
  ...
  listRequest(option) {
    ...
    this.settingsConfigService.post('/api/treaty_pay/my', option).subscribe((res: ApiData) => {
      console.log('listRequest res', res.data);
      if (res.code === 200) {
        console.log('非合约 我的支付');
