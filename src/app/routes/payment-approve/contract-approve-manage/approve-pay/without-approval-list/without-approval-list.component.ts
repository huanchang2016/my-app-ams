...
@Component({
  selector: 'app-without-approval-list',
  templateUrl: './without-approval-list.component.html',
  styles: []
})
export class WithoutApprovalListComponent implements OnInit {
// 单位id
...
getDataList() { // 获取单位下的数据
  this.loading = true;
  this.settingConfigService.get('/api/contract/pay/without_approval/my', this.pageOption).subscribe((res:ApiData) => {
    console.log(res, '未审批未通过的合同支付');
    this.loading = false;
    if(res.code === 200) {
