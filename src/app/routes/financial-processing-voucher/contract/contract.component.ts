...
@Component({
  selector: 'app-financial-processing-voucher-contract',
  templateUrl: './contract.component.html',
})
export class FinancialProcessingVoucherContractComponent implements OnInit {
  listOfData:any[] = [];
  ...
  getDataList(id:number = null) { // 获取单位下的数据
    this.loading = true;
    let url:string = id ?  `/api/contract_pay/finished/${id}` : '/api/contract_pay/finished/all';
    this.settingsConfigService.get(url, this.pageOption).subscribe((res:ApiData) => {
      console.log(res, '支付凭证： 合约支付管理');
      this.loading = false;
      if(res.code === 200) {
