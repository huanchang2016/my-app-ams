...
import { CostFormComponentComponent } from './cost-form-component/cost-form-component.component';
...
export class ProjectCostListComponent implements OnInit {
  ...
  createComponentModal(data:any = null): void {
    ...
    const modal = this.modalService.create({
      ...
      nzContent: CostFormComponentComponent,
  ...
  getDataList(id:number = this.companyId) { // 获取单位下的数据
    ...
    this.settingConfigService.get(`/api/cost/category/${id}`).subscribe((res:ApiData) => {
      ...
      this.loading = false;
      if(res.code === 200) {
        let data:any[] = res.data.cost_category;
