...
@Component({
  selector: 'app-my-apply-for-inprogress',
  templateUrl: './my-apply-for-inprogress.component.html',
  styles: []
})
export class MyApplyForInprogressComponent implements OnInit {
  ...
  getDataList() { // 获取单位下的数据
    this.loading = true;
    this.settingConfigService.get('/api/contract/pay/in_progress/my', this.pageOption).subscribe((res:ApiData) => {
      console.log(res);
      this.loading = false;
      if(res.code === 200) {
